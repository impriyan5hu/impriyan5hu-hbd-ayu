...
function GiftBox({ number, selected, disabled, onSelect }: { number: number; selected: boolean; disabled: boolean; onSelect: () => void }) {
  return (
    <Button
      type="button"
    ...
    >
      ...
      <span aria-hidden="true" className="gift-lid absolute left-2 top-7 z-20 block h-6 w-20 border-2 border-primary bg-secondary sm:left-3 sm:top-8 sm:h-7 sm:w-[6.5rem]">
      ...
      <span aria-hidden="true" className="gift-box absolute bottom-1 left-3 block h-14 w-[4.5rem] border-2 border-primary bg-muted sm:left-4 sm:h-[4.5rem] sm:w-24">
        <span className="absolute inset-y-0 left-1/2 w-4 -translate-x-1/2 bg-accent" />
...
export function GiftSelectionScreen({ onNext }: GiftSelectionScreenProps) {
  ...
  return (
    <section className="relative flex w-full max-w-[34rem] flex-col items-center px-1 py-4 text-center sm:px-5 sm:py-7">
      <span aria-hidden="true" className="absolute left-[8%] top-[8%] rotate-[-8deg] text-2xl text-primary sm:text-3xl">✦</span>
      <span aria-hidden="true" className="absolute right-[9%] top-[20%] rotate-12 text-xl text-primary sm:text-2xl">★</span>

      <h1 className="font-display text-4xl leading-none text-primary sm:text-5xl">Pick a gift 🎁</h1>
      <p className="mt-2 font-handwritten text-lg font-bold text-primary sm:text-xl">Choose any one!</p>
      ...
      <div className="mt-5 flex min-h-32 w-full flex-col items-center justify-start sm:mt-7">
        {selected ? (
          <div className="gift-message animate-fade-in flex flex-col items-center">
            <p aria-live="polite" className="max-w-[20rem] font-handwritten text-xl font-bold leading-snug text-primary sm:text-2xl">
              {selected.message}
            </p>
