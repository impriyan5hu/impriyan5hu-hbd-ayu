...
export function CakeWishScreen({ onNext }: CakeWishScreenProps) {
  ...
  return (
    <section className="relative flex w-full max-w-[27rem] flex-col items-center px-2 py-3 text-center sm:px-5 sm:py-5">
      <span
        aria-hidden="true"
        className="absolute left-[7%] top-[14%] rotate-[-9deg] text-2xl text-primary sm:text-3xl"
      >
        ✦
      </span>
      <span
        aria-hidden="true"
        className="absolute right-[8%] top-[31%] rotate-12 text-xl text-primary sm:text-2xl"
      >
        ★
      </span>

      <p className="font-handwritten text-lg font-bold text-primary sm:text-xl">
        It&apos;s time to
      </p>
      <h1 className="mt-1 font-display text-4xl leading-none text-primary sm:text-5xl">
        make a wish
      </h1>
      ...
      >
        <img
          src={birthdayCake}
          alt="A hand-drawn pink birthday cake with one candle"
        ...
        {!candleBlown ? (
          <span
            aria-hidden="true"
            className="candle-flame absolute left-1/2 top-[5.5%] h-9 w-6 -translate-x-1/2 bg-accent"
          />
        ) : (
      ...
      <p
        aria-live="polite"
        className="min-h-6 font-handwritten text-base font-bold text-primary sm:text-lg"
      >
        {candleBlown ? "Your wish is on its way!" : "Tap the flame to blow it out"}
      </p>

      <div className="mt-3 min-h-12 sm:mt-4">
        {candleBlown ? <NextButton onNext={onNext} /> : null}
      </div>
    </section>
  );
}
